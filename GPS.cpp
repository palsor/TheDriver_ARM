...
void GPS::init() {    
  // configure GPS
  bus.serialPrint(GPS_SERIAL_PORT, MTK_SET_BINARY);
  bus.serialPrint(GPS_SERIAL_PORT, MTK_OUTPUT_10HZ);
...
//
void GPS::readData() {
  while (bus.serialAvailable(GPS_SERIAL_PORT) > 0) {
    uint8 c = bus.serialRead(GPS_SERIAL_PORT);
